...
def get_output_map(version_map: Mapping[RedistName, Sequence[Version]]) -> Mapping[tuple[RedistName, Version], Path]:
    base_path: Path = Path(".") / "redist" / "data" / "custom"
    output_map: Mapping[tuple[RedistName, Version], Path] = {
        (redist_name, version): base_path / redist_name / f"{version}.json"
...
def main() -> None:
    ...
    nvidia_index: NvidiaIndex = NvidiaIndex.get_from_disk(
        Path(".") / "redist" / "data" / "nvidia", redist_names, args.version
    )
